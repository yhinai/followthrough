...
from .integrations import convex_event, elevenlabs, entity, linkup
...
class Crew:
    ...
    def process(self, run_id: str, text: str, classification: Classification, *, allow_owner_report: bool = True) -> dict[str, Any]:
        ...
        draft = {"policy": policy, "sent": False, "message": self._draft(entity(text), brief), "reason": "external sends remain approval-gated"}
        ...
        if self.settings.auto_send and allow_owner_report:
            try:
                ...
                self.store.add_step(run_id, "publisher", "failed", "owner-facing Discord report", {"reason": type(exc).__name__})
        else:
            self.store.add_step(run_id, "publisher", "skipped", "owner-facing Discord report", {"reason": "disabled by policy"})
    ...
    def _brief(self, text: str, classification: Classification, plan: dict[str, Any], research: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"headline": "A high-value conversation is ready for follow-through", "classification": classification.kind, "recommendation": plan.get("user_value", "Review the research and choose the next move."), "research": answer[:5000], "sources": research.get("sources", []), "next_action": "Review the follow-up draft; no external message was sent automatically.", "spoken_brief": f"Followthrough found a {classification.kind} signal. {plan.get('user_value', 'A research brief and safe draft are ready.')} No external message was sent without approval."}

    @staticmethod
    def _draft(subject: str, brief: dict[str, Any]) -> str:
        return f"Hi — following up on our conversation about {subject[:160]}. I researched it and found: {brief['recommendation']} Would Tuesday at 2 work for a quick follow-up?"
